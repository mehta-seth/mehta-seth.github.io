...
{/* Adit Mehta — first published essay.
    ...
    No H2s, by design. The ToC sidebar is gated on heading presence in
    EssayLayout, so this essay renders without ToC chrome — the essay
    column centers in the full viewport with no empty grid columns.
    ...
    text becomes a native browser tooltip on hover. The duplication is
    the cost of a zero-JS hover preview; small price for the behavior.
