...
{/* Adit Mehta — first published essay.
    ...
    A <FootnoteSection /> component sits between the body and the
    footnote items. It renders an asterism divider (⁂) followed by a
    "Footnotes" label. The label is rendered as a styled <p>, NOT an
    <h2>, so it never pollutes the ToC even when future essays use
    body H2s.
...
<Figure
  src={lalitPhoto}
  alt="Lalit at the table in his Udaipur studio."
/>
